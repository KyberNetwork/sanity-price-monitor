...
class TradesDownloader:
    ...
    async def download_all_trades(self):
        ...
        data_fetching_tasks = [
            self._read_data_from_web_file(trade_url) for trade_url in trade_urls
        ]
    ...
    async def _get_trade_urls(self):
        data = await network.get_response_content_from_get_request(
            url=TradesDownloader.TRADE_ARCHIVE_BASE_URL, format=DataFormat.TEXT
        )
    ...
    async def _read_data_from_web_file(self, trades_url):
        ...
        data = await network.get_response_content_from_get_request(
            url=trades_url, format=DataFormat.TEXT, timeout=240
        )

        trades = [
            normalize_trade_values(json.loads(line)) for line in data.splitlines()
        ]

        print(
            f"Downloading data from {trades_url} - finished with {len(trades)} trades"
        )
        return trades
...
class TradesFromCollectorArchive:
    ...
    async def download_all_trade_data_per_coin(self):
        ...
        trades_per_coin = {coin: [] for coin in self._coins}
...
def convert_values_to_uppercase(original_dict):
    return {key: value.upper() for key, value in original_dict.items()}
