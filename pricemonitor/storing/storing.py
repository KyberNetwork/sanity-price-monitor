...
class SanityContractUpdater:
    ...
    async def update_prices(self,
                            coin_price_data: List[PairPrice],
                            loop, force: bool = False
                            ) -> Optional[int]:
        previous_rates = await self._get_previous_rates(loop)  # type: Dict[Tuple[Coin, Coin], float]
    ...
    async def _get_pair_price_future(self, coin: Coin, loop) -> PairPrice:
        return PairPrice(pair=(coin, self._config.market), price=await self.get_rate(coin, loop))
    ...
    async def _get_previous_rates(self, loop) -> Dict[Tuple[Coin, Coin], float]:
        ...
        return {
            pair_price.pair: pair_price.price
            for pair_price in previous_rates
        }

    def _prepare_rates_for_update(self,
                                  previous_rates: Dict[Tuple[Coin, Coin], float],
                                  new_rates: List[PairPrice]
                                  ) -> List[PairPrice]:
        updates = []
        for pair_price in new_rates:
            if pair_price.price and self._should_update_price(
                    ...
                    previous_rate=self._get_previous_rate(coin=pair_price.pair[0],
                                                          market=pair_price.pair[1],
    ...
    @staticmethod
    def _should_update_price(coin: Coin,
                             market: Coin,
    ...
    @staticmethod
    def _get_previous_rate(coin: Coin, market: Coin, rates: Dict[Tuple[Coin, Coin], float]) -> Optional[float]:
        try:
    ...
    def format_coin_prices_for_setter(coin_price_data: List[PairPrice]) -> List:
        ...
        for pair_price in coin_price_data:
            ...
            if pair_price.price:
                ...
                rates.append(ContractRateArgumentsConverter.convert_price_to_contract_units(pair_price.price))
