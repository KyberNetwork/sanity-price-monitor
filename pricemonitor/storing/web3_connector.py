...
from pricemonitor.storing.node_errors import (
    PreviousTransactionPending,
    NonceAlreadySpent,
)
from pricemonitor.storing.web3_interface import (
...
class Web3Connector:
        ...
        for attempt in range(NUMBER_OF_ATTEMPTS_ON_FAILURE):
            try:
                rs = await self._wrap_sync_function(
                    call_function=self._web3_interface.call_const_function,
    ...
    async def call_remote_function(self, function_name, eth_args, loop):
        ...
        for attempt in range(NUMBER_OF_ATTEMPTS_ON_FAILURE):
            try:
                rs = await self._wrap_sync_function(
                    call_function=self._web3_interface.call_function,
                    function_name=function_name,
                    eth_args=eth_args,
                    loop=loop,
                ...
                log.debug(
                    "Previous transaction failed - pending tx with same nonce, sending with higher gas price"
                )

            except NonceAlreadySpent:
                use_increased_gas_price = False
                log.debug(
                    "Previous transaction failed - nonce used for committed tx, sending again"
                )
        ...
        log.info(
            f"{function_name}({eth_args})\n\t-> {rs} ({self._web3_interface.prepare_etherscan_url(rs)})"
        )
        return rs

    async def _wrap_sync_function(
        self, call_function, function_name, eth_args, loop, *args, **kwargs
    ):
        log.debug(
        ...
        try:
            ...
            raise Web3ConnectionError(
                msg, call_function, function_name, eth_args
            ) from e
